            ...
            try (BufferedReader reader = new BufferedReader(
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    if (regNo == null || name == null || branch == null) {
                        rowErrors.add(ImportResult.RowError.builder()
                                .row(rowNum).reason("Missing required fields").build());
                        skipped++;
                        continue;
                    }
                    ...
                    // ✅ Case-insensitive match — preserves canonical casing from config
                    String matchedBranch = validBranches.stream()
                    ...
                    if (matchedBranch == null) {
                        rowErrors.add(ImportResult.RowError.builder()
                                .row(rowNum)
                                .reason("Invalid branch '" + branch + "'. Allowed: " + validBranches)
                                .build());
                        skipped++;
                        continue;
                    }
                    ...
                    if (seenInBatch.contains(regNo)) {
                        rowErrors.add(ImportResult.RowError.builder()
                                .row(rowNum)
                                .reason("Duplicate regNo in file: " + regNo)
                                .build());
                        skipped++;
                        continue;
                    }
                    ...
                    toProcess.add(new ParsedRow(rowNum, regNo, name, matchedBranch));
                }
                ...
                for (ParsedRow row : toProcess) {
                    ...
                    if (student == null) {
                        rowErrors.add(ImportResult.RowError.builder()
                                .row(row.rowNum())
                                .reason("Failed to create student: " + row.regNo())
                                .build());
                        skipped++;
                        continue;
                    }
